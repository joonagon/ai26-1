...
def home():
    ...
    if search_text:
        ...
        return render_template("tagindex.html", search_text=search_text, tags_list=tags_list)

    return render_template("tagindex.html")
...
def search_tags():
    ...
    results = c.fetchall()
    return render_template("tagindex.html", results=results)
...
def generate_tags():
    ...
    c.execute("INSERT INTO tags (input_text, tags, created_at) VALUES (?, ?, ?)",
              (input_text, ", ".join(tags), created_at))
    conn.commit()

    return render_template("tagindex.html", tags=tags)
